...
const getOrderStatus = (order: Pick<UnifiedOrder, 'status' | 'order_status'>): string =>
  normalizeStatus(order.status || order.order_status);
...
const AdminOrders: React.FC = () => {
  ...
  const updateOrderStatus = async (order: UnifiedOrder, status: string) => {
    ...
    setForSource(prev =>
      prev.map(o => (o.id === orderId ? { ...o, status, order_status: status, payment_status: paymentStatus } : o))
    );
    setSelectedOrder(prev =>
      prev && prev.id === orderId && prev._source === source
        ? { ...prev, status, order_status: status, payment_status: paymentStatus }
        : prev
    );
    ...
    try {
      ...
      const previousSelected = previous.find(o => o.id === orderId);
      setSelectedOrder(prev =>
        prev && prev.id === orderId && prev._source === source && previousSelected
          ? previousSelected
          : prev
      );
  ...
  const filteredOrders = currentOrders.filter((order) => {
    ...
    const matchStatus = statusFilter === 'all' || orderStatus === normalizeStatus(statusFilter);
  ...
  const getStatusCount = (status: string) => {
    ...
    return currentOrders.filter((order) => getOrderStatus(order) === normalizeStatus(status)).length;
  };
